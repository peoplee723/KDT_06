...
def temp_minmax_mean():
    ...
    print(f'{start_year}년 부터 {end_year}년까지 {search_month}월의 기온 변화')
    print(f'{search_month}월 최저기온 평균\n{df_minmean_list}')
    print('\n')
    print(f'{search_month}월 최고기온 평균:\n{df_maxmean_list}')
    title=(f'{start_year}년부터 {end_year}까지 {search_month}의 기온 변화')
    drawgraph(title, year_list, df_maxmean_list, '최고기온', df_minmean_list,
              '최저기온')
temp_minmax_mean()