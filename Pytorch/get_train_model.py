...
def model_training(model, trainDL, testDL, optimizer, EPOCH: int, LIMIT: int, break_param: Literal['score', 'loss'],
                   type: Literal['reg', 'binary', 'muticlass'],optim_type: Literal['score', 'loss'], SAVE_PATH, SAVE_FILE, SAVE_NAME, 
                   save_type: Literal['all', 'param', 'None']=None, numcls: int|None= None):
    '''
    학습진행+ 모니터링+ 최적의 결과 저장
    ...
    for epoch in range(EPOCH):
        ...
        with torch.no_grad():
            for val_feature, val_target in testDL:
                ...
                # 손실
                loss= Lossfunc(pre_val, val_target.reshape(-1) if type=='muticlass' else val_target if type=='reg' else val_target)
                loss_val_total+=loss.item()

                # 평가
                score= Scorefunc(pre_val, val_target.reshape(-1).long() if type=='muticlass' else val_target)
        ...
        if save_type:
            ...
            #첫번째는 무조건 저장
                torch.save(save_type, SAVE_PATH+SAVE_FILE)  #파라미터
                torch.save(model, SAVE_PATH+SAVE_NAME)                 #전체
            else:
        ...
        if optim_type== 'score':
            lr_scheduler.step(score_val_total/len(testDL)) 
        elif optim_type== 'loss':
            lr_scheduler.step(loss_val_total/len(testDL))
...
def predicting(model, data, type: Literal['reg', 'binary', 'muticlass'], result: list| None= None):
