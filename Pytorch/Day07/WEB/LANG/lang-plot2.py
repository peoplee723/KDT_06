...
# (1) 알파벳 출현 빈도 데이터 수집
with open("../DATA/LANG/freq_train.json", mode="r", encoding="utf-8") as fp:
...
lang_dic = {}
if DEBUG:
    print('freq[0]["labels"] = {}'.format(freq["labels"]))

for i, lbl in enumerate(freq["labels"]):
    if DEBUG: print('i = {}\t lbl = {}'.format(i, lbl))

    fq = freq["freqs"][i]    # 트레인 용 데이터 추출
    if not (lbl in lang_dic):   # 레이블 언어 데이터 채우기
        lang_dic[lbl] = fq
        #continue
    else:
        #lang_dic[lbl] = (lang_dic[lbl] + fq)/2
