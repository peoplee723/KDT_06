...
while True:
    ...
    if not choice_main==1 and not choice_main==2: print(f'{choice_main}는 올바른 번호가 아닙니다');continue
        ...
        while True:
            ...
            if not check_data(choice_menu, 1) and 0<int(choice_menu)<7:
                print(f'{choice_menu}는 올바른 번호가 아닙니다')
                continue
            else: 
                choice_menu= int(choice_menu)
                order.append([main_menu[choice_menu]])
            # ing_loop=True
            # while ing_loop:
            #     # print('ing_loop')
            ...
            #             if extra== 1:                                    #재료 추가
            #                 ing.extend(ing_in[extra])
                       
            #             elif extra== 2:                                    #재료 빼기
            #                 main_menu=list(main_menu)
            #                 ing.extend(ing_out[extra])

            #             elif extra==3: continue
            
                    
            print('asdf')
            if len(order[1:-1]):                        #order에 
                    print('상품을 담아 주세요');continue
            if choice_menu==6:                          #결제하기
                print_take()
            ...
            if check_data(choice, 1):
                    choice= int(choice)
            else: print(f'{choice}는 올바른 번호가 아닙니다');continue
            if not choice_pay==1 and not choice_pay==2: print(f'{choice}는 올바른 번호가 아닙니다');continue
            elif choice_pay==1:
                main_menu.append('카드결제')
            elif choice_pay==2:
                main_menu.append('현금결제')